...
const BookingManagement = () => {
  ...
  const toOption = (item) => ({
    value: item.id,
    label: [item.code || item.customer_code || item.enquiry_code || item.quotation_code || item.name || item.customer_name || item.full_name || item.email || item.id].filter(Boolean).join(' - '),
    raw: item,
  });
  ...
  const chooseReference = (field, option) => {
    ...
    if (field === 'customer_id') updateForm('customer_name', raw.name || raw.full_name || raw.customer_name || '');
    if (field === 'customer_id') updateForm('mobile', raw.mobile || raw.phone || '');
    if (field === 'customer_id') updateForm('email', raw.email || '');
  };
